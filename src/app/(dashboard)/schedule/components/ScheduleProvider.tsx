...
import { ScheduleContextType, ScheduleState, ScheduleActions } from '../types/schedule.types';
 ...
 */
export function ScheduleProvider({ children, userId, readonly = false }: ScheduleProviderProps) {
   ...
   */
  const handleCourseMove = useCallback(async (
    courseId: string,
    fromSemester: number,
    fromPeriod: number,
    toSemester: number,
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to move course';
      dispatch({ 
   ...
   */
  const handleCourseRemoval = useCallback(async (enrollmentId: string) => {
    if (readonly) return;
  ...
  useEffect(() => {
    const handleAsyncAction = async () => {
      ...
      if (lastAction?.type === ScheduleActions.MOVE_COURSE && lastAction.payload) {
        const { courseId, fromSemester, fromPeriod, toSemester, toPeriod } = lastAction.payload;
        await handleCourseMove(courseId, fromSemester, fromPeriod, toSemester, toPeriod);
      }
      
      if (lastAction?.type === ScheduleActions.REMOVE_COURSE && lastAction.payload) {
