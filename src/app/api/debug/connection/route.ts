  ...
  try {
    ...
    const result = await withPrisma(
      async (prisma) => {
        ...
        const rawQueryResult: any[] =
          await prisma.$queryRaw`SELECT current_timestamp as server_time`;
    ...
    const response = {
      ...
      connection: {
        ...
        pooling: process.env.DATABASE_URL?.includes('connection_limit')
          ? 'enabled'
          : 'default',
        authenticated: !!user,
      },
      ...
      environment: {
        ...
        databaseUrlPrefix: process.env.DATABASE_URL
          ? maskDatabaseUrl(process.env.DATABASE_URL)?.split('?')[0]
          : undefined,
        directUrlPrefix: process.env.DIRECT_URL
          ? maskDatabaseUrl(process.env.DIRECT_URL)?.split('?')[0]
          : undefined,
