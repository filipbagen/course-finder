  ...
  try {
    ...
    const result = await withPrisma(async (prismaClient) => {
      ...
      if (existingEnrollment) {
        return { conflict: true, message: 'Enrollment already exists for this course and semester' };
      }

      // Check if user is enrolled in this course in a different semester
      const existingCourseEnrollment = await prismaClient.enrollment.findFirst({
        where: {
          userId: user.id,
          courseId: courseId,
        },
    ...
    const errorResponse = internalServerError('Failed to create enrollment. Please try again.');
    ...
    return NextResponse.json(errorResponseData, { status: errorResponse.status });
  }
}
...
export async function GET(
  ...
  try {
    ...
    return internalServerError(`Failed to fetch enrollments. Please try again. (Ref: ${errorRef})`);
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return internalServerError(`Failed to delete enrollment. Please try again. (Ref: ${errorRef})`);
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    const result = await withPrisma(async (prismaClient) => {
      ...
      const existingInTargetSemester = await prismaClient.enrollment.findFirst({
        where: {
          userId: user.id,
    ...
    return internalServerError(`Failed to update enrollment. Please try again. (Ref: ${errorRef})`);
  }
}
