...
const connectionPoolConfig = 
  process.env.NODE_ENV === 'production'
    ? {
        connection_limit: 5, // Maximum connections per serverless instance
        pool_timeout: 10, // 10 seconds
        connect_timeout: 10, // 10 seconds
        statement_timeout: 10000, // 10 seconds in ms
        idle_in_transaction_session_timeout: 10000, // 10 seconds in ms
      }
    : undefined;
...
const createPrismaClient = () => {
  console.log(`Creating new PrismaClient instance (NODE_ENV: ${process.env.NODE_ENV})`);
  ...
  while (attempt < maxRetries) {
    ...
    try {
      ...
      if (isConnectionError && attempt < maxRetries) {
        ...
        const backoff = Math.min(initialBackoff * Math.pow(2, attempt - 1) * jitter, maxBackoff);
        
        console.warn(
          `Database connection error (attempt ${attempt}/${maxRetries}), retrying in ${backoff.toFixed(0)}ms: ${error.message}`
        );
      ...
      if (error instanceof Error && error.name?.includes('Prisma') && !isConnectionError) {
